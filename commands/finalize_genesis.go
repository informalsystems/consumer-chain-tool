...
func getFinalizeCommandUsage() string {
	return fmt.Sprintf("%s [%s] [%s] [%s] [%s]",
		FinalizeGenesisCmdName, ConsumerChainId, MultisigAddress, ProposalId, ProviderNodeAddress)
}

func getFinalizeCommandExample() string {
	return fmt.Sprintf("%s %s %s %s %s %s", ToolName, FinalizeGenesisCmdName,
		"wasm", "wasm1ykqt29d4ekemh5pc0d2wdayxye8yqupttf6vyz", "1", "tcp://localhost:26657")
}

func getFinalizeGenesisLongDesc() string {
	return fmt.Sprintf(FinalizeGenesisLongDesc, ConsumerChainId, MultisigAddress, ProposalId, ProviderNodeAddress)
}
...
func NewFinalizeGenesisArgs(args []string) (*FinalizeGenesisArgs, error) {
	...
	proposalId := strings.TrimSpace(args[2])
	...
	providerNodeAddress := strings.TrimSpace(args[3])
